...
export const Solver = () => {
  ...
  const pickOption = (selectedOption: MechanicType) => {
    ...
    if (playerRole) {
      const playerParticipateInCurrentTurn = currentTurn?.solver.participants.includes(playerRole);
      ...
      if (playerParticipateInCurrentTurn) {
        const mechanicForPlayer = currentTurn?.solver.actions.find(action => action.role === playerRole);
    ...
    <div>
      ...
      {turn > 0 ? (
        <div>
          ...
          {selectedMechanic ? mechanicResult ? <div>✅ Great job</div> : <div>❌ Wrong</div> : null}
